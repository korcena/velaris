...
test.describe("Phase 6 — house & project templates", () => {
  test("instantiate a seeded house template and a project template", async ({ page, request }) => {
    ...
    // Pick the seeded "Engineering House" (default badge).
    await dialog.getByText("Engineering House", { exact: true }).click();
    const houseName = "Templated Engineering";
    ...
    await expect(manager.getByText("Engineering House").first()).toBeVisible();
